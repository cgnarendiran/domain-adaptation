...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    # define two different optimizers for discriminator and encoder:
    no_decay = ["bias", "LayerNorm.weight"]

    # adding separate optimizers for encoder, decoder and discriminator since they have to be learned adversarially
    parameter_groups = {"enc": model.bert.named_parameters(), "dec": model.cls.named_parameters(), "dis": model.dis.named_parameters()}

    optimizer_grouped_parameters = {k: [
        {
            "params": [p for n, p in param_gp if not any(nd in n for nd in no_decay)],
    ...
    for k, v in parameter_groups.items():
        if (
            ...
            and os.path.isfile(os.path.join(args.model_name_or_path, k+"_optimizer.pt"))
            and os.path.isfile(os.path.join(args.model_name_or_path, k+"_scheduler.pt"))
        ):
            # Load in optimizer and scheduler states
            optimizers[k].load_state_dict(torch.load(os.path.join(args.model_name_or_path, k+"_optimizer.pt")))
            schedulers[k].load_state_dict(torch.load(os.path.join(args.model_name_or_path, k+"_scheduler.pt")))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    for k in loss:
                        ...
                        wandb.log({k+" train loss": (tr_loss[k] - logging_loss[k]) / args.logging_steps})
                        logging_loss[k] = tr_loss[k]
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    for k in optimizers:
                        torch.save(optimizers[k].state_dict(), os.path.join(output_dir, k+"_optimizer.pt"))
                        torch.save(schedulers[k].state_dict(), os.path.join(output_dir, k+"_scheduler.pt"))
    ...
    return global_step, tr_loss["mlm"] / global_step
